...
//@property (nonatomic, strong) WBPopContentView *contentView;
@end
...
-(instancetype)initWithOrigin:(CGPoint)origin Width:(CGFloat)width Height:(CGFloat)height Direction:(WBArrowDirection)direction Type:(WBPopContent_TYPE)type
{
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)])
    {
        ...
        WBPopContentView *contentView = [[WBPopContentView alloc]initWithFrame:CGRectMake(origin.x, origin.y, width,0) andType:type];
        contentView.backgroundColor = hexCOLOR(@"#ffe9c9");
...
-(void)drawRect:(CGRect)rect
{
    ...
    else if (_direction==WBArrowDirectionUp2)
    {
        CGFloat startX = self.origin.x;
        CGFloat startY = self.origin.y;
        CGContextMoveToPoint(context, startX, startY+2);//设置起点
